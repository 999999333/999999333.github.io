    ...
    USE [L50_SB_Vitek];
    execute('create view "sfa"."int_pepsi_documents_closed_enhanced__dbt_tmp" as
        with document as (
	select * from "L50_SB_Vitek"."sfa"."stg_sap__documents_closed"
),

calendar as (
    select * from "L50_SB_Vitek"."sfa"."stg_dwh__calendar"
),

entity as (
    select * from "L50_SB_Vitek"."sfa"."stg_dwh__dim_entities"
), 

country as (
	select * from "L50_SB_Vitek"."sfa"."stg_dwh__dim_countries"
),

product as (
	select * from "L50_SB_Vitek"."sfa"."stg_sap__products"
),

customer as (
	select * from "L50_SB_Vitek"."sfa"."stg_dwh__dim_customers"
),

fx as (
    select * from "L50_SB_Vitek"."sfa"."stg_dwh__fx_rates"
),

call_representative as (
    select * from "L50_SB_Vitek"."sfa"."stg_dwh__dim_call_representatives"
),
