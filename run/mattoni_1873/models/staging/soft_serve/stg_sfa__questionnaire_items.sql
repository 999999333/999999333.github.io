    ...
    USE [L50_SB_Vitek];
    execute('create view "sfa"."stg_sfa__questionnaire_items__dbt_tmp" as
        with source as (
    select * from "L50_SB_Vitek"."snapshots"."sfa__document_items_snapshot"
),
renamed as (
    select
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(max), concat(coalesce(cast(Country_Code as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(Item_ID as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
 ...
 as questionnaire_section_id,
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(max), concat(coalesce(cast(Country_Code as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(StandartDocumentItemId as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
