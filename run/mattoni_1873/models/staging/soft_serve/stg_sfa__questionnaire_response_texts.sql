    ...
    USE [L50_SB_Vitek];
    execute('create view "sfa"."stg_sfa__questionnaire_response_texts__dbt_tmp" as
        with source as (
      select * from "L01_Stage"."sfa"."dbo_ResponsesSingleD_v"
),
renamed as (
    select
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(max), concat(coalesce(cast(Country_Code as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(ResponseValueId as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
