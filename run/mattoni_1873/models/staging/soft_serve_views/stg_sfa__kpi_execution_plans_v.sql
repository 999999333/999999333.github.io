    ...
    USE [L50_SB_Vitek];
    execute('create view "sfa"."stg_sfa__kpi_execution_plans_v__dbt_tmp" as
        with source as (
    select * from "L01_Stage"."sfa"."ps_KPIExecutionPlan_v"
),
...
renamed as (
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(max), concat(coalesce(cast(Country_Code as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(KpiId as VARCHAR(MAX)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
 as kpi_id,
        "KpiId" as kpi_key,
