    ...
    EXEC('create view "dbt"."stg_sfa__organization_structures_v__dbt_tmp" as with source as (
...
renamed as (
 ...
 as organizational_structure_id,
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(8000), concat(coalesce(cast(Country_Code as VARCHAR(8000)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(NodeId as VARCHAR(8000)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
