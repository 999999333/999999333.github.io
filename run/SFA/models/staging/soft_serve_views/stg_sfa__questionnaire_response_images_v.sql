    ...
    EXEC('create view "dbt"."stg_sfa__questionnaire_response_images_v__dbt_tmp" as with source as (
...
renamed as (
 ...
 as sfa_file_id,
    ...
    lower(convert(varchar(50), hashbytes(''md5'', coalesce(convert(varchar(8000), concat(coalesce(cast(Country_Code as VARCHAR(8000)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(Response_ID as VARCHAR(8000)), ''_dbt_utils_surrogate_key_null_''), ''-'', coalesce(cast(Object_ID as VARCHAR(8000)), ''_dbt_utils_surrogate_key_null_''))), '''')), 2))
 as image_id,
